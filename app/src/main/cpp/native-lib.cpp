...
int dailyCredits = 100;
bool socialUnlocked = false;
std::chrono::steady_clock::time_point unlockExpiry;

bool isMatch(const std::vector<std::string>& list, const std::string& pkg) {
    return std::find(list.begin(), list.end(), pkg) != list.end();
}

// BlockerService checkAppStatus
extern "C" JNIEXPORT jint JNICALL
Java_com_android_neverGiveUp_BlockerService_checkAppStatus(
    ...
    if (isMatch(CREDIT_GATED, pkg)) {
        if (socialUnlocked) {
            if (std::chrono::steady_clock::now() > unlockExpiry) {
...
// RelentlessAccessibility checkAppStatus (ONLY ONE VERSION!)
extern "C" JNIEXPORT jint JNICALL
Java_com_android_neverGiveUp_RelentlessAccessibility_checkAppStatus(
        JNIEnv *env, jobject, jstring jPackage) {
    if (!jPackage) return 0;
    const char* raw = env->GetStringUTFChars(jPackage, nullptr);
    std::string pkg(raw);
...
// CreditGateActivity methods
extern "C" JNIEXPORT jboolean JNICALL
Java_com_android_neverGiveUp_CreditGateActivity_nativePurchaseSocialPass(
        JNIEnv*, jobject) {
    if (dailyCredits >= 50 && !socialUnlocked) {
        dailyCredits -= 50;
        socialUnlocked = true;
        unlockExpiry = std::chrono::steady_clock::now() + std::chrono::minutes(30);
        return JNI_TRUE;
    }
    return JNI_FALSE;
}
...
extern "C" JNIEXPORT jint JNICALL
Java_com_android_neverGiveUp_CreditGateActivity_nativeGetCredits(
        JNIEnv*, jobject) {
    return dailyCredits;
}

// BlockerService nativeResetDailyCredits
extern "C" JNIEXPORT void JNICALL
Java_com_android_neverGiveUp_BlockerService_nativeResetDailyCredits(
        JNIEnv*, jobject) {
    dailyCredits = 100;
    socialUnlocked = false;
}
...
extern "C" JNIEXPORT jint JNICALL
Java_com_android_neverGiveUp_MainActivity_nativeGetCredits(
        JNIEnv*, jobject) {
    return dailyCredits;
}

extern "C" JNIEXPORT jlong JNICALL
Java_com_android_neverGiveUp_MainActivity_nativeGetUnlockSecondsRemaining(
        JNIEnv*, jobject) {
    if (!socialUnlocked) return 0L;
    auto now = std::chrono::steady_clock::now();
    if (now >= unlockExpiry) {
        socialUnlocked = false;
        return 0L;
    }
    return (jlong)std::chrono::duration_cast<std::chrono::seconds>(
            unlockExpiry - now).count();
}

extern "C" JNIEXPORT jboolean JNICALL
Java_com_android_neverGiveUp_MainActivity_nativePurchaseSocialPass(
        JNIEnv*, jobject) {
    if (dailyCredits >= 50 && !socialUnlocked) {
        dailyCredits -= 50;
        socialUnlocked = true;
...
// DashboardFragment methods
extern "C" JNIEXPORT jint JNICALL
Java_com_android_neverGiveUp_DashboardFragment_nativeGetCredits(JNIEnv*, jobject) {
    return dailyCredits;
}

extern "C" JNIEXPORT jlong JNICALL
Java_com_android_neverGiveUp_DashboardFragment_nativeGetUnlockSecondsRemaining(JNIEnv*, jobject) {
    if (!socialUnlocked) return 0L;
    auto now = std::chrono::steady_clock::now();
    if (now >= unlockExpiry) { socialUnlocked = false; return 0L; }
    return (jlong)std::chrono::duration_cast<std::chrono::seconds>(unlockExpiry - now).count();
}

extern "C" JNIEXPORT jboolean JNICALL
Java_com_android_neverGiveUp_DashboardFragment_nativePurchaseSocialPass(JNIEnv*, jobject) {
    if (dailyCredits >= 50 && !socialUnlocked) {
        dailyCredits -= 50;
        socialUnlocked = true;
        unlockExpiry = std::chrono::steady_clock::now() + std::chrono::minutes(30);
