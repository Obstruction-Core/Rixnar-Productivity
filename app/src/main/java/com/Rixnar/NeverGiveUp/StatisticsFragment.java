...
public class StatisticsFragment extends Fragment {
    ...
    private TextView tvMonthYearHeader, tvMeterPercentage, tvMeterLabel, tvSelectedDateLabel, tvSelectedDateStatus;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (!prefs.contains("tracking_init_date_string")) {
            // Default to Jan 01 of the current year so backfilled slips (earlier dates) are included in metrics.
            Calendar jan1 = Calendar.getInstance();
            jan1.set(Calendar.MONTH, Calendar.JANUARY);
            jan1.set(Calendar.DAY_OF_MONTH, 1);
            String initialDate = dateFormat.format(jan1.getTime());
            prefs.edit().putString("tracking_init_date_string", initialDate).commit();
        }
        ...
        // Date picker is now triggered by tapping the month/year header ("March 2026" etc.)
        tvMonthYearHeader.setOnClickListener(v -> showSafeDatePickerDialog());

        btnCommitSlip.setOnClickListener(v -> {
            Set<String> slips = new HashSet<>(prefs.getStringSet("immutable_slips_pool", new HashSet<>()));
            
            // Gap filling logic: if we commit a slip, fill the gap since the last recorded slip
            String latestBefore = null;
            for (String s : slips) {
                if (s != null && s.compareTo(selectedDateString) < 0) {
                    if (latestBefore == null || s.compareTo(latestBefore) > 0) {
                        latestBefore = s;
                    }
                }
            }
            ...
            if (latestBefore != null) {
                try {
                    ...
                    fillCal.setTime(dateFormat.parse(latestBefore));
                    fillCal.add(Calendar.DAY_OF_YEAR, 1);
                    String fillStr = dateFormat.format(fillCal.getTime());
                    while (fillStr.compareTo(selectedDateString) <= 0) {
                        slips.add(fillStr);
                        fillCal.add(Calendar.DAY_OF_YEAR, 1);
                        fillStr = dateFormat.format(fillCal.getTime());
                    }
            ...
            // Commit synchronously and save to internal storage to prevent state reset on fragment switch
            prefs.edit().putStringSet("immutable_slips_pool", slips).commit();
            PersistenceHelper.saveToInternalStorage(getActivity());

            // If the user backfills older slips, move init date back so metrics count them.
            String todayString = dateFormat.format(System.currentTimeMillis());
            ensureInitDateCoversSlips(todayString, slips);

            Toast.makeText(getActivity(), "Slips locked and saved!", Toast.LENGTH_SHORT).show();
    ...
    private String ensureInitDateCoversSlips(String todayString, Set<String> slips) {
        String currentInit = prefs.getString("tracking_init_date_string", null);

        if (currentInit == null || currentInit.trim().isEmpty()) {
            Calendar jan1 = Calendar.getInstance();
            jan1.set(Calendar.MONTH, Calendar.JANUARY);
            jan1.set(Calendar.DAY_OF_MONTH, 1);
            currentInit = dateFormat.format(jan1.getTime());
        }

        String earliest = currentInit;
        if (slips != null) {
            for (String s : slips) {
        ...
        // Safety: never allow init to drift into the future.
        if (earliest.compareTo(todayString) > 0) {
            earliest = todayString;
        }

        if (!earliest.equals(currentInit)) {
            prefs.edit().putString("tracking_init_date_string", earliest).commit();
        }
                    ...
                    if (tv != null && "today".equals(tv.getTag())) {
                        ...
                        if (bg != null) {
                            bg.setStroke((int) (2 * getResources().getDisplayMetrics().density),
                                    Color.argb((int) (flashAlpha * 255), 76, 175, 80)); // brand_green
                        }
                    }
                }
    ...
    private void populatePhysicalCalendar() {
        ...
        String initStr = ensureInitDateCoversSlips(todayStr, slips);
        ...
        for (Calendar cellCal : daysList) {
            ...
            // Keep calendar compact so the whole screen fits without scrolling on most devices.
    ...
    private void showSafeDatePickerDialog() {
        ...
        datePickerDialog.setButton(DatePickerDialog.BUTTON_POSITIVE, "CONFIRM DATE", (dialog, which) -> {
            ...
            // Instantly refresh layout + calculations
            updateSelectedDayUI();
        ...
        if (datePickerDialog.getWindow() != null) {
            ...
            dialogBg.setColor(androidx.core.content.ContextCompat.getColor(getActivity(), R.color.surface_card)); 
            dialogBg.setCornerRadius(28 * getResources().getDisplayMetrics().density); // Smooth organic curvature
            dialogBg.setStroke((int)(1.5f * getResources().getDisplayMetrics().density), androidx.core.content.ContextCompat.getColor(getActivity(), R.color.surface_stroke));
    ...
    private void calculateMetrics() {
        String todayString = dateFormat.format(System.currentTimeMillis());
        Set<String> slips = prefs.getStringSet("immutable_slips_pool", new HashSet<>());
        String initDateString = ensureInitDateCoversSlips(todayString, slips);
